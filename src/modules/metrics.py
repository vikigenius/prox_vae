...
class CosineSimilarity(Metric):
    ...
    def __call__(self,  # type: ignore
                 t1: torch.Tensor,
                 t2: torch.Tensor):
        assert t1.dim() == 2 and t2.dim() == 2
        t1, t2 = self.unwrap_to_tensors(t1, t2)
        sim_score = ((t1*t2).sum(dim=1)/(t1.norm(dim=1)*t2.norm(dim=1))).mean()
        self._similarity += sim_score
        self._count += 1
    ...
    def get_metric(self, reset: bool = False):
        ...
        """
        average_value = self._similarity / self._count if self._count > 0 else 0
        if reset:
            self.reset()
