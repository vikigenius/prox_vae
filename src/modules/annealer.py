...
class TanhAnnealedWeight(AnnealedWeight):
    """
    This class anneals weights in a tanh fashin.
    """
...
@LossWeight.register("smoothed_sigmoid_annealed")
class SmoothedSigmoidAnnealedWeight(AnnealedWeight):
    """
    This class anneals weights in a sigmoid fashion.
    """
    def __init__(self, slope: float, margin: float, min_weight: float = 0.0,
    ...
    def _get_weight(self):
        ...
        weight = 1/(1 + np.exp(-self.slope*(self.iteration - self.margin)))
        weight = shifted_max*weight + self.min_weight
        return weight
