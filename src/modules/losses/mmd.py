...
def mmd(z_tilde: torch.Tensor, z: torch.Tensor, z_var: float = 1.0):
    ...
    out = im_kernel_sum(z, z, z_var, exclude_diag=True).div(n*(n-1)) + \
        im_kernel_sum(
            z_tilde, z_tilde, z_var, exclude_diag=True).div(n*(n-1)) + \
...
def im_kernel_sum(z1: torch.Tensor, z2: torch.Tensor, z_var: float, exclude_diag=True):
    r"""Calculate sum of sample-wise measures of inverse multiquadratics
    kernel described in the WAE paper.
    Args:
        z1 (Tensor): batch of samples from a multivariate gaussian \
            distribution with scalar variance of z_var.
        z2 (Tensor): batch of samples from another multivariate gaussian \
            distribution with scalar variance of z_var.
        exclude_diag (bool): whether to exclude diagonal kernel measures \
             before sum it all.
    """
